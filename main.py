...
# external resources code - https://www.relataly.com/content-based-movie-recommender-using-python/4294/
...
df_meta = df_meta.set_index(
    df_meta['id'].str.strip().replace(',', '').astype(int))
...
df_keywords = pd.read_csv(
    'data/keywords.csv', low_memory=False, encoding='UTF-8')
df_keywords = df_keywords.set_index('id')

# merge everything into a single dataframe
df_k_c = df_keywords.merge(df_credits, left_index=True, right_on='id')
df = df_k_c.merge(df_meta[['release_date', 'genres',
                  'overview', 'title']], left_index=True, right_on='id')
df.head(3)
...
df_movies['keywords'] = df['keywords'].apply(
...
df_movies['release_date'] = pd.to_datetime(df['release_date'], errors='coerce').apply(
    lambda x: str(x).split('-')[0] if x != np.nan else np.nan)

# extract the actors
df_movies['cast'] = df['cast'].apply(lambda x: [i['name'] for i in eval(x)])
df_movies['cast'] = df_movies['cast'].apply(
    lambda x: ' '.join([i.replace(" ", "") for i in x]))

# extract genres
df_movies['genres'] = df['genres'].apply(
    lambda x: [i['name'] for i in eval(x)])
...
df_movies['tags'] = df_movies['keywords'] + df_movies['cast'] + \
    ' '+df_movies['genres']+' '+df_movies['release_date']
...
count_matrix = pd.DataFrame(
    vectorized_data.toarray(), index=df_movies['tags'].index.tolist())
print(count_matrix)
